#!/usr/bin/env python3
"""unpack_images.py — decode a ak_images.json bundle (pulled from Claude Design
...
this script writes them to disk. Re-run whenever the image set changes.
    ...
    python3 scripts/unpack_images.py [path/to/ak_images.json]
    (defaults to ~/Downloads/ak_images.json)
"""
import base64
import json
import sys
...
# Default to the NEWEST ak_images*.json (Chrome appends "(1)", "(2)", … to
# repeat downloads, so a fixed name would read a stale bundle).
if len(sys.argv) > 1:
    src = Path(sys.argv[1])
else:
    matches = sorted(Path.home().glob("Downloads/ak_images*.json"), key=lambda p: p.stat().st_mtime)
    if not matches:
        raise SystemExit("No ak_images*.json found in ~/Downloads")
    src = matches[-1]
